 ...
 * - 문자열에 대해서 유효성검사/검색/대체등을 처리하기위한 표현식
 * - 조건/반복처리를 내부적으로 진행해서 간결하게 작성가능
 * - 문법자체 숙지 어려운편.
 * - 언어독립적
 */
document.querySelector('#btn1').addEventListener('click', () => {
    ...
    console.log("a1b2c3d".split(re)); // 구분자인 숫자는 제외되고 문자배열만 돌려줌

    // String#replace(re, newString): string
    console.log("abc-123".replace(re, "*")); // 숫자에 해당하는 부분이 *로 변환 abc-***
    console.log("abc1def2ghi".replace(re, "<$&>")); // abc<1>def<2>ghi 매칭된 값을 쓰고 싶을때, $& 대체문자 사용
    ...
    console.log('a1b2c3d'.search(re)); // 1 첫번째 매칭결과
    console.log('xyzㅋㅋㅋㅋㅋㅋ'.search(re)); // -1 매칭되는게 없을때
});
...
document.querySelector('#btn5').addEventListener('click', (e) => {
    const src = `ABCDEFGhijkmn 가나다라마바사ㅋㅋㅎㅎㅓㅓㅏㅏ 1234567890 !@#$%^&*()\t\n`;
    const area = e.target.nextElementSibling;
    ...
    area.innerHTML += `<p>${src.replace(/[abcdefghijkmnopqurstuvwxyz]/g, '<mark>$&</mark>')}</p>`;
...
document.querySelector('#btn6').addEventListener('click', (e) => {
    const src = `ABCDEFGhijkmn 가나다라마바사ㅋㅋㅎㅎㅓㅓㅏㅏ\t\n 1234567890 !@#$%^&*()_+| \t\n`;
    const area = e.target.nextElementSibling;
...
document.querySelector('#btn10').addEventListener('click', (e) => {
    ...
    console.log(re.test('1234456')); // false
...
document.querySelector('#password').addEventListener('blur', (e) => {
    ...
    // 영문자 포함
    else if(!regexps[1].test(value)){
        alert('비밀번호는 영문자를 하나이상 포함해야 합니다.');
        return;
    }
    // 숫자 포함
    else if(!regexps[2].test(value)){
        alert('비밀번호는 숫자를 하나이상 포함해야 합니다.');
        return;
    }
    // 특수문자(!@#$%^&*()) 포함
    else if(!regexps[3].test(value)){
...
document.querySelector('#btn-ssn').addEventListener('click', (e) => {
    const ssn = e.target.previousElementSibling;
...
//     const dashCheck = inputVal.split(/-/);
    
//     if (dashCheck.length !== 2) {
//         alert('-를 포함한 주민번호를 입력하세요.');
//         return;
//     }
    
//     const ssnFirst = /^[0-9]{6}$/g;
//     const ssnLast = /^[0-9]{7}$/g;
//     let bool1;
//     let bool2;
//     dashCheck.forEach((ssn, i) => {
//         if (i == 0)
//             bool1 = ssnFirst.test(ssn);
//         else
//             bool2 = ssnLast.test(ssn);
//     });
